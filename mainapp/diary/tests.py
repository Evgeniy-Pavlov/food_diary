...
class TestUserRegister(TestCase):
    ...
    def test_register_invalid(self):
        ...
        response_1 = self.client.post('/api/register', json.dumps(data_1), content_type='application/json')
        ...
        response_2 = self.client.post('/api/register', json.dumps(data_2), content_type='application/json')
        ...
        response_3 = self.client.post('/api/register', json.dumps(data_3), content_type='application/json')
...
class TestUserStatFodDay(TestCase):
    ...
    def test_stat_from_day_without_stat(self):
        data = {'username': 'test_user', 'email': 'test_user@mail.com', 'password': 'Qwerty777!!'}
        response_reg = self.client.post('/api/register', json.dumps(data), content_type='application/json')
    ...
    def test_user_foodstat_add(self):
        data = {'username': 'test_user_food', 'email': 'test_user_food@mail.com', 'password': 'Qwerty777!!'}
        response_reg = self.client.post('/api/register', json.dumps(data), content_type='application/json')
        username = response_reg.json()['username']
        user = UserBase.objects.get(username=username)
        food = DirectoryFood.objects.create(name = 'Test_food', caloric= 100, fat = 200, protein = 300, carbon = 400)
...
class TestFoodStatAdd(TestCase):
    def setUp(self):
        ...
        self.food_1 = DirectoryFood.objects.create(name = 'Test_food', caloric= 100, fat = 200, protein = 300, carbon = 400)
        self.food_2 = DirectoryFood.objects.create(name = 'Test_food_2', caloric= 200, fat = 400, protein = 600, carbon = 800)
        user_auth = self.client.post('/api/auth', json.dumps({'username': self.user.username, 'password': data['password']}), content_type='application/json')
        self.token = user_auth.json()['access']
        today = datetime.datetime.today()
        self.time = today.strftime('%Y-%m-%d')

    def test_userstat_add(self):
        response = self.client.post('/api/user/userstat/add', json.dumps({'user': self.user.id, 'calories_burned': self.food_1.caloric, 'date': self.time}), \
        ...
        response = self.client.post('/api/user/userstat/add', json.dumps({'user': self.user.id, 'calories_burned': self.food_2.caloric, 'date': self.time}), \
            content_type='application/json', headers={'Authorization': f'Bearer {self.token}'})
        self.assertEqual(response.status_code, HTTPStatus.OK)
        result = response.json()
        self.assertEqual(result['user'], self.user.id)
